...
function displayPageAction(tab) {
  ...
  browser.storage.local.get(key).then(result => {
    ...
    console.log(
      "page action disabled " + record.disabled + " for tab " + tab.id
    );
...
function archiveTab(tab) {
  ...
  browser.bookmarks
    .create({ title: tab.title, url: tab.url, parentId: folder_guid })
    .then(() => {
      let key = tab.url.split("#")[0];
      browser.storage.local.remove(key);
...
function checkTab(tab, age_limit) {
  ...
  if (!isSupportedProtocol(tab.url)) {
    console.debug(
      "Tab " + tab.id + " url scheme is unsupported (" + tab.url + ")"
    );
    return;
  }
  ...
  browser.storage.local.get(key).then(result => {
    ...
    console.debug(
      "check tab " + tab.id + ", disabled: " + record.disabled + ", age: " + age
    );
...
function tabSweep() {
  ...
  browser.storage.local.get("max_age").then(result => {
    let max_age =
      (result.hasOwnProperty("max_age") ? result.max_age : 1) * 60 * 1000;
